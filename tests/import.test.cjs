...
test('embedded assets are complete data URLs',()=>{const s={window:{}};vm.runInNewContext(read('src/assets.js'),s,{timeout:2000});const a=s.window.TimerAssets;assert.ok(Object.keys(a).length>=27);for(const [name,url]of Object.entries(a)){const m=/^data:([^;]+);base64,([A-Za-z0-9+/=]+)$/.exec(url);assert.ok(m,name);assert.ok(Buffer.from(m[2],'base64').length>50,name);}assert.equal(a['chew-8.webp'],a['squirrel-ready.webp']);assert.ok(a['apple.svg']);assert.ok(a['elephant-ready.webp']);for(let i=0;i<9;i++)assert.ok(a['elephant-chew-'+i+'.webp']);assert.equal(a['elephant-chew-8.webp'],a['elephant-chew-7.webp']);for(const k of ['tidy','meal','bath','out','brush','sleep'])assert.ok(a['next-'+k+'.webp']);});
